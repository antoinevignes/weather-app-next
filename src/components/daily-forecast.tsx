...
export function DailyForecast({ weatherData }: { weatherData: WeatherData }) {
  return (
    <ScrollArea className="mx-auto mb-4">
      <div className="flex gap-6 mb-6">
        {weatherData.daily.time.map((day: Date, index: number) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className="text-center border-2 rounded-md border-transparent hover:border-[#4c4f69] dark:hover:border-[#cdd6f4] transition duration-200 cursor-pointer px-2 md:px-3.5"
            >
